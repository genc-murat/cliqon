        ...
        AuthMethod::Password => {
            ...
            if let Err(e) = session.userauth_password(&profile.username, pwd) {
                ...
                if methods.contains("keyboard-interactive") {
                    struct AutoPrompt<'a> { pwd: &'a str }
                    impl<'a> ssh2::KeyboardInteractivePrompt for AutoPrompt<'a> {
                        fn prompt(&mut self, _name: &str, _instructions: &str, prompts: &[ssh2::Prompt]) -> Vec<String> {
                    ...
                    let res = session.userauth_keyboard_interactive(
                        &profile.username,
                        &mut prompt_handler,
                    );
                    if res.is_ok() {
