...
impl TunnelService {
    ...
    pub fn start_tunnel(&self, session: &Session, config: TunnelConfig, session_id: String) -> Result<()> {
        ...
        match config.tunnel_type {
                                ...
                                if let Ok(mut channel) = channel_res {
                                    ...
                                    thread::spawn(move || {
                                        ...
                                        while let Ok(n) = channel_clone.read(&mut buf) {
                                            if n == 0 { break; }
                                            if tcp_stream_clone.write_all(&buf[..n]).is_err() { break; }
                                        }
                                    });
                                }
                            }
            ...
            TunnelType::Remote => {
                let local_port = config.local_port;
                let remote_host = config.remote_host.clone().unwrap_or_else(|| "0.0.0.0".to_string());
                let remote_port = config.remote_port.unwrap_or(8080);
                
                let (mut listener, _) = session.channel_forward_listen(remote_port, Some(&remote_host), None)?;
                ...
                thread::spawn(move || {
                    while running_flag.load(Ordering::Relaxed) {
                        ...
                        match listener.accept() {
                            Ok(mut channel) => {
                                if let Ok(mut tcp_stream) = TcpStream::connect(format!("127.0.0.1:{}", local_port)) {
                                    let mut tcp_stream_clone = tcp_stream.try_clone().unwrap();
                                    let mut channel_clone = channel.clone();
                        ...
                        match stream {
                            Ok(mut tcp_stream) => {
                                ...
                                thread::spawn(move || {
                                    ...
                                    if tcp_stream.read(&mut buf).unwrap_or(0) < 3 { return; }
                                    if buf[0] != 0x05 { return; } // Only SOCKS5 supported
                                    
                                    // 2. Reply no auth
                                    if tcp_stream.write_all(&[0x05, 0x00]).is_err() { return; }
                                    
                                    // 3. Read connection request
                                    let n = tcp_stream.read(&mut buf).unwrap_or(0);
                                    if n < 10 || buf[0] != 0x05 || buf[1] != 0x01 { return; } // Only TCP connect (0x01)
                                    ...
                                    if atyp == 0x01 { // IPv4
                                        ...
                                        let _ = tcp_stream.write_all(&[0x05, 0x08, 0x00, 0x01, 0, 0, 0, 0, 0, 0]); // Address type not supported
                                        return;
                                    }
                                    
                                    // 4. Connect via SSH channel_direct_tcpip
                                    if let Ok(mut channel) = session_for_thread.channel_direct_tcpip(&host, port, None) {
                                        // Reply success
                                        if tcp_stream.write_all(&[0x05, 0x00, 0x00, 0x01, 0, 0, 0, 0, 0, 0]).is_err() { return; }
                                        ...
                                        thread::spawn(move || {
                                            ...
                                            while let Ok(n) = tcp_stream.read(&mut b) {
                                                if n == 0 { break; }
                                                if channel.write_all(&b[..n]).is_err() { break; }
                                            }
                                            let _ = channel.send_eof();
                                        });
                                        ...
                                        while let Ok(n) = channel_clone.read(&mut b) {
                                            if n == 0 { break; }
                                            if tcp_stream_clone.write_all(&b[..n]).is_err() { break; }
                                        }
                                    } else {
                                        // Reply general failure
                                        let _ = tcp_stream.write_all(&[0x05, 0x01, 0x00, 0x01, 0, 0, 0, 0, 0, 0]);
                                    }
                                });
                            },
                            Err(ref e) if e.kind() == std::io::ErrorKind::WouldBlock => {
        ...
        tunnels.insert(tunnel_id, ActiveTunnel { config, session_id, is_running });
