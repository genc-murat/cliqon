...
type ToolCategory = 'diagnostics' | 'status' | 'infrastructure' | 'security' | 'advanced';
...
export const NetworkTools: React.FC<NetworkToolsProps> = ({ profile, sessionId, onClose, isEmbedded }) => {
    ...
    const [activeCategory, setActiveCategory] = useState<ToolCategory>('diagnostics');
    ...
    const tabs: { id: ToolTab; label: string; icon: React.ReactNode }[] = [
        ...
        { id: 'nmap', label: 'Nmap Scan', icon: <Shield size={13} /> },
        { id: 'whois', label: 'Whois', icon: <Search size={13} /> },
        { id: 'mtr', label: 'MTR', icon: <Activity size={13} /> },
        { id: 'tracepath', label: 'Tracepath', icon: <Navigation2 size={13} /> },
        { id: 'nslookup', label: 'Nslookup', icon: <Search size={13} /> },
        { id: 'netstat', label: 'Netstat -rn', icon: <List size={13} /> },
        { id: 'fail2ban_status', label: 'Fail2Ban', icon: <ShieldAlert size={13} /> },
        { id: 'hostname_info', label: 'Hostname', icon: <Info size={13} /> },
        { id: 'active_users', label: 'Users', icon: <UserCheck size={13} /> },
        { id: 'open_files', label: 'Lsof', icon: <FileText size={13} /> },
        { id: 'curl_timing', label: 'Curl Timing', icon: <Timer size={13} /> },
        { id: 'dns_config', label: 'Resolv.conf', icon: <FileJson size={13} /> },
        { id: 'hosts_file', label: 'Hosts File', icon: <FileText size={13} /> },
        { id: 'uptime', label: 'Uptime', icon: <Clock size={13} /> },
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 flex overflow-hidden">
                {/* Category Sidebar */}
                <div className="w-12 flex flex-col items-center py-2 gap-2 border-r border-[var(--border-color)] bg-[var(--bg-primary)]/20 shrink-0">
                    {CATEGORIES.map(cat => (
                        <button
                            key={cat.id}
                            onClick={() => {
                                setActiveCategory(cat.id);
                                if (!cat.tools.includes(activeTab)) {
                                    setActiveTab(cat.tools[0]);
                                    resetResults();
                                }
                            }}
                            title={cat.label}
                            className={`p-2 rounded-lg transition-all ${activeCategory === cat.id
                ...
                <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div className="flex items-center justify-between px-3 py-1.5 border-b border-[var(--border-color)] bg-[var(--bg-primary)] shrink-0 select-none">
                        <div className="flex items-center gap-1 overflow-x-auto hide-scrollbar">
                            {!isEmbedded && <Globe size={14} className="text-[var(--accent-color)] mr-2" />}
                            <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider mr-2 shrink-0">
                                {CATEGORIES.find(c => c.id === activeCategory)?.label}
                            </span>
                            {tabs.filter(t => activeCategoryTools.includes(t.id)).map(t => (
                                <button
                                    key={t.id}
                                    onClick={() => {
                                        setActiveTab(t.id);
                    ...
                    <div className="flex-1 overflow-y-auto p-4 min-h-0">
                        ...
                        {/* Raw fallback for data with no specific parser/view */}
                        {!loading && rawOutput && (
                            ((activeTab === 'dns' && dnsResult.length === 0) ||
                        ...
                        {/* Raw Output for New Tools */}
                        {['nmap', 'whois', 'mtr', 'tracepath', 'nslookup', 'netstat', 'fail2ban_status', 'hostname_info', 'active_users', 'open_files', 'curl_timing', 'dns_config', 'hosts_file', 'uptime', 'disk_usage', 'memory_usage', 'last_logins'].includes(activeTab) && rawOutput && !loading && (
                            <div className="bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl p-4 overflow-x-auto">
                                <pre className="font-mono text-xs text-[var(--text-main)] whitespace-pre-wrap">{rawOutput}</pre>
                            </div>
                        )}
                    </div>
                </div>
